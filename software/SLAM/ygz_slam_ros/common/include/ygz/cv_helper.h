...
public:
    ...
    vector<cv::Point3f> Points3d2Points3f(vector<cv::Point3d> input_points)
    {
        ...
        {
            output_points.emplace_back(p.x, p.y, p.z);
        }
    ...
    vector<cv::Point2d> Points2f2Points2d(vector<cv::Point2f> input_points)
    {
        ...
        for(auto& p : input_points)
        {
            ...
            output_points.emplace_back(p.x, p.y);
        }
...
//                                  true, true);
      ...
      for( int i = 0; i < matches_1.size(); i++ )
      {
        if( matches_1[i].distance <= 2*min_dist || matches_1[i].distance< 10)
        {
      ...
      LOG(INFO)<<"final_good_kps_left size: "<<final_good_kps_left.size()<<endl;
