...
class A_star(object):
    ...
    def __success(self, current_obj,center_point_can_gaol):
        if center_point_can_gaol:
            if current_obj.get_point() == self.end_pos:
                return True
        elif not center_point_can_gaol:
    ...
    def find_path(self, start_pos, obstacle_pos_list):
        ...
        center_point_can_gaol = self.is_valid(self.end_pos,obstacle_pos_list)   #判断中心点能否到达，如果中心点不能到达，则判断机身能否到达
        path = []
        count = 0
        ...
        while self.openlist:
            ...
            count += 1
            #current_obj = min(self.openlist, key=lambda p:p.G+p.H)
            #current_obj = self.openlist[0]
            # for node in self.openlist:
            #     if (node.G+node.H)<=(current_obj.G+current_obj.H): 
            #         if node.H <= current_obj.H:        
            #             current_obj = node

            current_obj = heapq.heappop(self.openlist)[1]
            #print ('len(self.openlist)',len(self.openlist))

            #if current_obj.get_point() == self.end_pos:
            if self.__success(current_obj,center_point_can_gaol):   #判断是否到达
                while current_obj.parent:
                    path.append(current_obj.get_point())
                    current_obj = current_obj.parent
                path.append(current_obj.get_point())
                print ('count:',count)
                print "extend:",len(self.openlist)+len(self.clostlist)
                return path[::-1]
            #self.openlist.remove(current_obj)
            self.clostlist.add(current_obj)
            #self.clostlist.append(current_obj)
    ...
    def extend_round(self, current_obj, obstacle_pos_list):
        ...
        for x,y,z in self.movement_list:    
            new_point = (x+current_obj.get_point()[0],y+current_obj.get_point()[1],z+current_obj.get_point()[2])
            node_obj = Node(new_point)
            if not self.is_valid(new_point,obstacle_pos_list):
                continue
            if self.is_in_closelist(new_point):
                continue
    ...
    def is_in_closelist(self,p):
        for node in self.clostlist:
    ...
    def is_valid(self,pt,obstacle_map):
        ...
        for item in aircraft_obj['aircraft_points']:
            aircraft_points.append(
                (item[0]+pt[0],item[1]+pt[1],item[2]+pt[2])
            )
