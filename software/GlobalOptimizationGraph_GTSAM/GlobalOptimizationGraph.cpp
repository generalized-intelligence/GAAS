...
void GlobalOptimizationGraph::addBlockSLAM(int msg_index)
{
    ...
    auto elapsed_time = slam_time_current - gps_time_current;
    double elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(elapsed_time).count();

    // if elapsed time since last GPS signal arrival time is greater than 1000ms,
    // the GPS is considered as "lost"
    if(elapsed_ms > 1000)
    {
        gps_lost = true;
    }
    ...
    LOG(INFO)<<"Elapsed_ms since last GPS signal: "<<elapsed_ms<<endl;
    LOG(INFO)<<"Is gps_lost? : "<<gps_lost<<endl;
    // ----------------------------------------------------------
...
//        LOG(INFO)<<"diff_yaw: \n"<<diff_yaw<<endl;
        ...
        if(gps_lost)
        {
            ...
            auto _SE3_slam = PoseStampedToSE3(SLAM_msg);
            auto _SE3_converted = mRelativeTransformSLAM2Fused * _SE3_slam;
            SLAM_msg = SE3ToPoseStamped(_SE3_converted);
...
//            Values currentEstimate = p_isam->calculateEstimate();
            ...
            // fetch values from optimizer
            {
                ...
                current_status.ret_val_t[2] = current_height.x(); //y is not used here
                current_status.header_ = SLAM_msg.header;
                current_status.innerID_of_GOG = this->slam_vertex_index-1;

                // keep track of optimized result and slam pose
                mOptimizedPose.block(0,0, 3, 3) = current_status.ret_val_R.toRotationMatrix();
                mOptimizedPose.block(0,3, 3, 1) = current_status.ret_val_t;

                Eigen::Quaterniond slam_q = Eigen::Quaterniond(SLAM_msg.pose.orientation.w,
                                                               SLAM_msg.pose.orientation.x,
                                                               SLAM_msg.pose.orientation.y,
                                                               SLAM_msg.pose.orientation.z);
                mCurrentSlamPose.block(0,0, 3, 3) = slam_q.toRotationMatrix();
                mCurrentSlamPose.block(0,3, 3, 1) = Eigen::Vector3d(SLAM_msg.pose.orientation.x,
                                                                     SLAM_msg.pose.orientation.y,
                                                                     SLAM_msg.pose.orientation.z);

                LOG(INFO)<<"current_status.ret_val_R: \n"<<newx<<", "<<newy<<", "<<newz<<", "<<neww<<endl;
                LOG(INFO)<<"current_status.ret_val_R.toRotationMatrix(): \n"<<current_status.ret_val_R.toRotationMatrix()<<endl;
                LOG(INFO)<<"current_status.ret_val_t.t(): \n"<<current_status.ret_val_t.transpose()<<endl;
                LOG(INFO)<<"mOptimizedPose: \n"<<mOptimizedPose<<endl;

                // mRelativeTransformOS: from raw slam frame to optimized result frame
                // mOptimizedPose: from body1 to world, Twb1
                // mCurrentSlamPose: from body2 to world, Twb2
                mRelativeTransformSLAM2Fused = mOptimizedPose.inverse() * mCurrentSlamPose;

                LOG(INFO)<<"mRelativeTransformSLAM2Fused: \n"<<mRelativeTransformSLAM2Fused<<endl;
