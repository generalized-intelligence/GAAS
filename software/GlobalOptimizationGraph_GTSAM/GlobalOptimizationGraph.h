...
public:
    ...
    template<typename T>
    inline geometry_msgs::PoseStamped SE3ToPoseStamped(const T& pose)
    {
        auto _R = pose.block(0,0,3,3);
        Eigen::Quaterniond _q(_R);
        geometry_msgs::PoseStamped _pose;
        _pose.pose.position.x = pose(0, 3);
        _pose.pose.position.y = pose(1, 3);
        _pose.pose.position.z = pose(2, 3);
    ...
    inline Eigen::Matrix4d PoseStampedToSE3(geometry_msgs::PoseStamped _pose)
    {
        Eigen::Matrix4d _SE3;
        _SE3.block(0,0, 3, 3) = Eigen::Quaterniond(_pose.pose.orientation.w,
                                                   _pose.pose.orientation.x,
                                                   _pose.pose.orientation.y,
                                                   _pose.pose.orientation.z).toRotationMatrix();
        _SE3.block(0,3, 3, 1) = Eigen::Vector3d(_pose.pose.position.x,
...
private:
    ...
    Eigen::Matrix4d mOptimizedPose;
    Eigen::Matrix4d mCurrentSlamPose;
    Eigen::Matrix4d mRelativeTransformSLAM2Fused;
};
