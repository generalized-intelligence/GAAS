...
#ifndef MESHGEN_PIPELINE_H
#define MESHGEN_PIPELINE_H
...
void do_cvPyrLK(InputOutputArray prev,InputOutputArray next,vector<Point2f>& p2d_v_prev,vector<Point2f>& p2d_v_next,vector<unsigned char>& track_success_v,vector<float>& err,bool do_backward_check)
{
    ...
    cv::findFundamentalMat(p2d_v_prev,p2d_v_next,fundamental_match_success);
...
shared_ptr<PointDetected> extractCamKeyPoints_splited(cvMatT& Img)
{
    ...
    cvMatT out_feats; //TODO, not used?
