...
reserve_num = int(sys.argv[1])
def get_gps_position_by_lines(line):
    if line.find('Adding gps measurement:')>0:
        print line
        print line.split('measurement:')[-1].split(',')
        return map(float,line.split('measurement:')[-1].split(',')[-3:])
    return None

def get_slam_position_by_lines(line):
    #if line.find('[SLAM_INFO]')>=0 and line.find("slam position")>0:
    if line.find('Original SLAM xyz:')>=0:
        print line
        print line.split('xyz:')[-1].split(',')
        return map(float,line.split('xyz:')[-1].split(',')[-3:])
    return None

def get_optimized_position_by_lines(line):
    if line.find('Current NODE ESTIMATED Position for visualizer:')>=0:
        print line
        print line.split('Current NODE ESTIMATED Position for visualizer:')[-1].split(',')
...
def get_check_slam_data(check_file):
    ...
    for line in check_file:
        ...
        if res:
            ...
            zs.append(res[2])
            #ts.append(res[3])
    return xs[:reserve_num],ys[:reserve_num],zs[:reserve_num]#,ts
...
def get_check_optimized_pos_data(check_file):
    ...
    for line in check_file:
        ...
        if res:
            ...
            ys.append(res[1])
            zs.append(res[2])
    return xs[:reserve_num],ys[:reserve_num],zs[:reserve_num]
