...
private:
    ...
    const int max_kf_count = 2;//debug.
    int cam_count;
...
//        }
        ...
        *pResult = optimizer.optimize();//TODO:换成optimizer.optimizeSafely(),避免出现异常.
        ...
        if(this->reproj_db.frameTable.query(lastFrameID)->optimization_valid)//当优化成功时:
        {
            ...
            for(auto key_value = pResult->begin(); key_value != pResult->end(); ++key_value)
            {
                ...
                {//对所有的Frame:
                    ...
                    //}
                    pFrame->setRotationAndTranslation(rot.matrix(),trans.vector());
                    LOG(WARNING)<<"Frame"<<x_index<<"optimized!"<<endl;
                }
        ...
        if(USE_IMU_INFO)
        {
            ...
            {
                //使用filter 过滤这两个值.
                auto v = pResult->at(Symbol('V',last_optimized_frame_id)).cast<Vector3>();
                this->reproj_db.prev_state = NavState(pResult->at(Symbol('F',last_optimized_frame_id)).cast<Pose3>(),v);
                LOG(WARNING)<<"Current velocity:"<<v[0]<<","<<v[1]<<","<<v[2]<<endl;
...
//    }
    ...
    //this_frame_valid:当前帧是否有效.
    {
        ...
        trackAndKeepReprojectionDBForFrame(pCurrentFrame,track_states);//开始追踪.
        ...
        if(pCurrentFrame->frame_id >0)//根据追踪结果,分析是否需要关键帧.
        {
        ...
        {//创建关键帧
            ...
            {
                upgradeOrdinaryFrameToKeyFrameStereosReservingPreviousKPs(pCurrentFrame);//前端 从已更新帧补充特征点.
            }
            ...
            LOG(INFO)<<";"<<endl;
            pSWRes = optimizeFactorGraph(pSlidingWindGraph,pSWInitialEstimate,7,pCurrentKF->frame_id);
            //第六步 第二次优化.
