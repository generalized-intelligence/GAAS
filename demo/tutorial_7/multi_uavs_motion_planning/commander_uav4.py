...
class Commander:
    def __init__(self):
        rospy.init_node("uav4_commander_node")
        rate = rospy.Rate(20)
        self.position_target_pub = rospy.Publisher('/uav4/gi/set_pose/position', PoseStamped, queue_size=10)
        self.velocity_target_pub = rospy.Publisher('/uav4/gi/set_pose/velocity', Vector3Stamped, queue_size=10)
        self.yaw_target_pub = rospy.Publisher('/uav4/gi/set_pose/orientation', Float32, queue_size=10)
        self.custom_activity_pub = rospy.Publisher('/uav4/gi/set_activity/type', String, queue_size=10)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        con.move(vx=(math.pi-2)*math.cos(theta),vy=(math.pi-2)*math.sin(theta))
        time.sleep(0.02)
        j=j+1
        if j==100:
            i=i+1
